  ...
  try {
    ...
    for (let i = 1; i < playersData.length; i++) { // Start from 1 to skip header
      ...
      if (playerID && firstName && lastName) {
        ...
        playerStatsMap[fullName] = { goals: 0, assists: 0, pim: 0 };
      } else {
    ...
    for (let i = 1; i < playersData.length; i++) { // Start from 1 to skip header
      ...
      if (firstName && lastName) {
        ...
        if (stats) {
          ...
          ptsCounts.push([stats.goals + stats.assists]); // Corrected typo
          ...
          // Player not found in gameEvents; set goals, assists, and PIM to 0
          goalCounts.push([0]);
          assistCounts.push([0]);
          pimCounts.push([0]);
          ptsCounts.push([0]);
          Logger.log(`No stats found for ${fullName}. Setting goals, assists, and PIM to 0.`);
        }
      } else {
        // If first or last name is missing, set goals, assists, and PIM to 0
        goalCounts.push([0]);
        assistCounts.push([0]);
        pimCounts.push([0]);
        ptsCounts.push([0]);
    ...
    const goalsRange = playersSheet.getRange(startRow, GOALS_COL + 1, goalCounts.length, 1); // Column H is 8
    goalsRange.setValues(goalCounts);
    
    // Update Assists (Column I)
    const assistsRange = playersSheet.getRange(startRow, ASSISTS_COL + 1, assistCounts.length, 1); // Column I is 9
    assistsRange.setValues(assistCounts);
    
    // Update Points (Column J)
    const pointsRange = playersSheet.getRange(startRow, PTS_COL + 1, ptsCounts.length, 1); // Column J is 10
    pointsRange.setValues(ptsCounts);
    
    // Update PIM (Column K)
    const pimRange = playersSheet.getRange(startRow, PIM_COL + 1, pimCounts.length, 1); // Column K is 11
    ...
    SpreadsheetApp.getUi().alert('Player goals, assists, points, and penalty minutes have been updated successfully!');
